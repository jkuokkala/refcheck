...
function check_references($input, $opts, $lang = 'en') {
	global $ERRSTR;
	...
	foreach ($input as $line) {
		...
		if ($in_refs) {
			preg_match($reflist_regex, $line, $matches, PREG_UNMATCHED_AS_NULL);
			...
			if ($matches) {
				...
				if ($auths) {
					...
					$auths = preg_split('/\s+'. $authorsep .'\s+|\s+(?=et\s+al\.|ym\.?|jt\.?|u\.a\.)/u', $auths);
				...
				if ($m) {
					$m = preg_match('/^\s*((?:(?:[^,.=]+)(?:,\s*(?:[^.=]+))?)(?:\s+'. $authorsep .'\s+(?:(?:[^,.=]+)(?:,\s*(?:[^.=]+))?))*)/u', $matches[1], $matches);
					if ($m) {
						$auths = $matches[1];
						$year = '';
						...
						if ($auths) {
							$auths = preg_split('/\s+'. $authorsep .'\s+/', $auths);
							$auths = array_map(function($a) {
								return preg_split('/,\s*/', $a, 2);
							}, $auths);
						}
			...
			preg_match_all('/\b(?<![.-])((?:(?<=....\(|(?:(?:.\b[Ss]ee|[Ee]\.g\.|\.g\.,|.\b[Cc]f\.|.\b[Kk]s\.|\Bsim\.|[Vv]rt\.|.\b[Nn]t\.|...;)\s))(?:(?:[Dd][aei]|[Tt]e|[Vv]an\ [Dd]er|[Vv][ao]n)\s+)?[A-ZÅÄÖÜČŠŽ][A-\x{1FFE}\'’-]+?,\s*)*(?:[A-ZÅÄÖÜČŠŽ]\.\s*)*(?:(?:[Dd][aei]|[Tt]e|[Vv]an\ [Dd]er|[Vv][ao]n)\s+)?[A-ZÅÄÖÜČŠŽ][A-\x{1FFE}\'’-]+?(?:\s+(?:et\ al\.?|ym\.?|jt\.?|u\.a\.)|(?:\s*'. $authorsep .'\s*(?:(?:[Dd][aei]|[Tt]e|[Vv]an\ [Dd]er|[Vv][ao]n)\s+)?[A-ZÅÄÖÜČŠŽ][A-\x{1FFE}\'’-]+?)+)?)(?:[\'’]s)?\s*\(?(?:(?:e\.g\.|esim\.|mm\.)\s+)?((?:(?:[12][0-9]{3}(?:[–-][0-9]+)?[a-z]?(?:\s+\[[12][0-9]{3}(?:[–-][0-9]+)?\])?|(?:\(?(?:forthcoming|in\ press|in\ preparation|tulossa|painossa)\)?))(?<=\w|\])(?!\w)(?::\s*[0-9IVXivx]+(?:[ ,–-]+[0-9IVXivx]+)*)?(?:;\s+)?)+|(?:(?:[0-9]{1,2}|[IVX]+)?(?::\s*[0-9IVXivx]+(?:[ ,–-]+[0-9IVXivx]+)*|:?\s*s\.\s*v\.\s*[A-\x{1FFE}*-]+(?:[ ,–-]+[A-\x{1FFE}*-]+)*)(?:;\s+)?))/u', $line, $citcands, PREG_SET_ORDER);
			// (See more readable versions of the regexes in Python version)
			foreach ($citcands as $citcand) {
				$auths = $citcand[1];
				if (strpos($auths, ',') !== false && strpos($auths, $authorsep_pr) === false) {
					$auths = trim(substr($auths, strrpos($auths, ',') + 1));
				}
				...
				foreach ($auths as $i => $auth) {
					...
					} elseif (preg_match('/\s+(?:et\s+al\.|ym\.?|jt\.?|u\.a\.|[A-ZÅÄÖÜČŠŽ][a-zåäöüčšž]*\.)/u', $auth)) {
						$auths[$i] = preg_split('/\s+(?=et\s+al\.|ym\.?|jt\.?|u\.a\.|[A-ZÅÄÖÜČŠŽ][a-zåäöüčšž]*\.)/u', $auth);
					} else {
						$auths[$i] = array($auth);
					}
				}
